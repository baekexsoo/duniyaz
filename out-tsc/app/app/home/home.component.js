var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Component } from '@angular/core';
import { UtilisateurService } from '../../providers/utilisateur/utilisateur.service';
import { Router } from '@angular/router';
var HomeComponent = /** @class */ (function () {
    // lien = new Constant();
    function HomeComponent(User, router) {
        this.User = User;
        this.router = router;
        this.e_mail = "";
        this.pass = "";
        this.warning = "";
        this.loader = false;
    }
    HomeComponent.prototype.ngOnInit = function () {
    };
    HomeComponent.prototype.con = function () {
        var _this = this;
        this.loader = true; //c'est ici qu'on impose au loader de s'afficher 
        this.warning = ""; // Pour réinitialiser un warning !!!! IMPORTANT 
        this.User.connect(this.e_mail, this.pass).subscribe(function (response) {
            _this.data_conne = response;
            var duniya = {
                "usermail": _this.e_mail
            };
            sessionStorage.setItem("duniya", JSON.stringify(duniya));
            var compte = {
                "usermail": _this.e_mail
            };
            sessionStorage.setItem("compte", compte.usermail);
            _this.loader = false; // a la récuperation de la reponse on désactive le loader !!!!!!!!!!!
            _this.router.navigate(['/prediction']);
        }, function (err) {
            console.log(err);
            if (err.status === 500) {
                _this.warning = "Identifiants incorrects.";
            }
            else {
                _this.warning = "Une erreur est survenue. Veuillez reessayer";
            }
            _this.loader = false;
        });
    };
    HomeComponent = __decorate([
        Component({
            selector: 'app-home',
            templateUrl: './home.component.html',
            styleUrls: ['./home.component.css']
        }),
        __metadata("design:paramtypes", [UtilisateurService, Router])
    ], HomeComponent);
    return HomeComponent;
}());
export { HomeComponent };
//# sourceMappingURL=home.component.js.map